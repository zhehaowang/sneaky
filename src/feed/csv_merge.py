...
def merge_csvs(csvs, outfile_name, populate_no_match):
    ...
    for f in csvs:
        ...
        if not keys:
            ...
            print("dropping {} keys in {} not present in current intersection".format(len(set_keys - in_both), f))
            print("dropping {} keys in current intersection".format(len(keys - in_both)))
            keys = in_both
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--populate_no_match",
        action='store_true',
        help=("fill in entry in result csv even if one input doesn't have entry matching style_id"))
    parser.add_argument(
        "--output",
